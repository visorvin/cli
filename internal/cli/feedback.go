...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VISOR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VISOR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "visor/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.visor/feedback.jsonl.
When ` + "`VISOR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VISOR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  visor feedback list
  visor feedback list --limit 5
  visor feedback list --json`,
