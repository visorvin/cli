...
In local mode: searches locally synced data only.`,
  ...
  visor search "error timeout"

  # Force local search only
  visor search "payment failed" --data-source local

  # Search a specific resource type locally
  visor search "critical" --type transactions --data-source local

  # JSON output for piping
  visor search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'visor sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/visor/data.db)")
