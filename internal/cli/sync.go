...
Exit codes & warnings:
  ...
  visor sync --resources channels,messages
  ...
  visor sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  visor sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "dealers":
		return db.UpsertDealers(data)
	case "dealers_listings":
		return db.UpsertDealersListings(data)
	case "listings":
