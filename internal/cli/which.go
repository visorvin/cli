...
var whichIndex = []whichEntry{
	...
	{Command: "dealers list", Description: "Returns public dealer summaries ordered by active listing count. Listing counts use VIN-level listing deduplication and ownership assignment, so they can differ from raw dealer feed counts. Use dealer_id, dealer_group_id, state, country, type, make, and q to narrow results.", Group: "dealers"},
	...
	{Command: "listings list", Description: "Returns active, sold, or historical snapshot listing summaries using stable snake_case fields. Public inventory is deduplicated by VIN and assigned to the dealer Freeway believes owns the listing, so dealer-scoped counts can differ from raw dealer feed counts. Unknown query parameters fail closed with validation_error / unknown_query_parameter; unsupported closed-vocabulary values fail with validation_error.", Group: "listings"},
	...
	{Command: "vins get", Description: "Returns the current or latest known record for a VIN. Unknown VINs return 404 not_found_error; known VINs can return status: missing.", Group: "vins"},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  visor which "stale tickets"
  visor which "bottleneck"
  visor which --limit 1 "send message"
  visor which                                # list the full capability index`,
